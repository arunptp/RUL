...
async function onClickTrainModel(){
  ...
  let logHtml = document.getElementById("div_traininglog").innerHTML;
  logHtml = "<div>Model trained</div>" + logHtml;
  document.getElementById("div_traininglog").innerHTML = logHtml;
...
function onClickPredict() {
  ...
  let timestamps_c = data_raw.map(function (val) {
    return val['timestamp'];
  }).splice(window_size + Math.floor(trainingsize / 100 * outputs.length), data_raw.length);

  let sma = sma_vec.map(function (val) { return val['avg']; });
  let prices = data_raw.map(function (val) { return val['price']; });

  let graph_plot = document.getElementById('div_predict_graph');
...
async function predictFailure() {
  ...
  let input  = [[0.        , 0.        , 0.43150684, 0.51141554, 0.5813953 ,
      0.1883372 , 0.457814  , 0.        , 0.375     , 0.        ,
      0.        , 0.48264983, 0.6272727 , 0.64670914, 0.5260971 ,
      0.46497855, 0.        , 1.        , 0.45564517, 0.5121951 ,
      0.12007663, 0.45625   , 0.7692308 , 0.        ],
     [0.        , 0.        , 0.44520548, 0.60273975, 0.44186047,
      0.10314342, 0.5206136 , 0.        , 0.5       , 0.        ,
      0.        , 0.43533123, 0.48181817, 0.5021939 , 0.5035056 ,
      0.43046764, 0.        , 1.        , 0.5342742 , 0.4390244 ,
      0.2240613 , 0.44375   , 0.61538464, 0.        ],
     [0.        , 0.        , 0.44520548, 0.43607306, 0.44186047,
      0.16297553, 0.22818792, 0.        , 0.625     , 0.        ,
      0.        , 0.45741326, 0.6454545 , 0.6666667 , 0.689691  ,
      0.54053503, 0.        , 1.        , 0.49596775, 0.58536583,
      0.09226054, 0.525     , 0.6923077 , 0.        ],
     [0.        , 0.        , 0.42465752, 0.5       , 0.39534885,
      0.19119486, 0.48609778, 0.        , 0.5       , 0.        ,
      0.        , 0.37223974, 0.46363637, 0.7565464 , 0.37366918,
      0.48682868, 0.        , 1.        , 0.6592742 , 0.6097561 ,
      0.17264368, 0.66875   , 0.53846157, 0.        ],
     [0.        , 0.        , 0.45890412, 0.5045662 , 0.5116279 ,
   ...
   // document.getElementById('status').innerHTML = 'Model Not Loaded';
   // const probab = pred(input);
   // const predictedResults = model.predict(tf.tensor2d(input),dtype=float32);

  //const probab = predict_fun(input);
  import * as tf from '@tensorflow/tfjs';
  const tfmodel = await tf.loadLayersModel('https://raw.githubusercontent.com/arunptp/Residual-Useful-Life/master/model.json');
  //const probab = tfmodel.predict(tf.tensor2d(input),dtype=float32);
  document.getElementById('status').innerHTML = 'Model Loaded';
  $('#gaugeDemo .gauge-arrow').trigger('updateGauge', 40);
  document.getElementById("prob").innerHTML = tfmodel.summary() + "%";

}

async function pred_fun(input) {


    const model = await tf.models.modelFromJSON('/model/model.json');
    const probab = model.predict(tf.tensor2d(input),dtype=float32).dataSync();
    return 10;
...
function fetchSensorData() {
  ...
  if(sensorid == 'Sensor 1'){
    $("#div_linegraph_data_title").text("Engine Id: A9-01 / Sensor 1 (S4)");
    let values  = [1400.6 , 1403.14, 1404.2 , 1401.87, 1406.22, 1398.37, 1397.77,
         1400.97, 1394.8 , 1400.46, 1400.64, 1400.15, 1400.83, 1399.16,
         1402.13, 1404.5 , 1399.95, 1396.12, 1400.35, 1405.23, 1398.13,
         1400.57, 1394.75, 1398.81, 1399.39, 1396.07, 1401.84, 1399.13,
         1400.99, 1396.08, 1396.9 , 1400.36, 1402.43, 1400.48, 1403.  ,
         1405.54, 1398.9 , 1399.5 , 1400.69, 1401.77, 1403.21, 1401.38,
         1406.05, 1401.2 , 1399.16, 1401.27, 1402.89, 1399.16, 1397.76,
         1403.62, 1400.42, 1395.57, 1402.26, 1401.2 , 1401.58, 1400.76,
         1402.79, 1397.99, 1403.51, 1403.07, 1405.52, 1399.54, 1395.38,
         1400.11, 1403.24, 1399.52, 1403.02, 1404.42, 1401.24, 1402.78,
         1403.09, 1406.81, 1397.93, 1401.48, 1394.82, 1398.94, 1400.97,
         1404.1 , 1403.93, 1393.58, 1402.04, 1399.17, 1406.38, 1406.6 ,
         1405.96, 1403.56, 1413.1 , 1406.09, 1402.22, 1412.93, 1404.76,
         1400.77, 1408.48, 1404.94, 1410.1 , 1395.16, 1407.81, 1404.56,
         1406.13, 1411.35, 1403.41, 1403.42, 1403.21, 1406.25, 1403.22,
         1404.45, 1407.73, 1399.47, 1409.1 , 1401.27, 1408.11, 1401.84,
         1406.59, 1402.97, 1406.6 , 1401.99, 1409.35, 1409.34, 1404.46,
         1407.68, 1411.9 , 1413.97, 1403.04, 1402.28, 1406.42, 1418.5 ,
         1399.49, 1407.06, 1408.42, 1412.25, 1413.25, 1405.76, 1410.11,
         1402.67, 1406.83, 1404.39, 1405.15, 1413.34, 1412.6 , 1408.36,
         1415.25, 1413.42, 1404.21, 1405.76, 1409.63, 1408.82, 1409.96,
         1411.89, 1410.7 , 1409.22, 1411.94, 1420.83, 1414.56, 1416.07,
         1414.89, 1421.23, 1412.47, 1410.09, 1420.37, 1418.08, 1417.31,
         1423.48, 1421.09, 1416.76, 1408.09, 1422.37, 1414.72, 1415.7 ,
         1418.05, 1417.41, 1414.99, 1413.73, 1425.27, 1422.58, 1422.52,
         1422.73, 1423.99, 1424.65, 1429.45, 1417.14, 1420.64, 1426.62,
         1419.03, 1432.52, 1424.4 , 1426.3 , 1427.27, 1422.78, 1428.01,
         1425.95, 1425.77, 1427.2 ];
    ...
    $("#div_linegraph_data_title").text("Engine Id: A9-01 / Sensor 2 (S14)");
  ...
  else if(sensorid == 'Sensor 3'){
    $("#div_linegraph_data_title").text("Engine Id: A9-01 / Sensor 3 (S11)");
...
else if(sensorid == 'Sensor 4'){
    $("#div_linegraph_data_title").text("Engine Id: A9-01 / Sensor 4 (S7)");
  ...
  else if(sensorid == 'Sensor 5'){
      $("#div_linegraph_data_title").text("Engine Id: A9-01 / Sensor 4 (S2)");
...
function getTestData() {
  ...
  if(testdata == 'Sample 1'){
    $("#div_linegraph_data_titletest").text("Engine Id: A9-01 / Sensor 1 (S4)");
    let values  = [[0.        , 0.        , 0.43150684, 0.51141554, 0.5813953 ,
        0.1883372 , 0.457814  , 0.        , 0.375     , 0.        ,
        0.        , 0.48264983, 0.6272727 , 0.64670914, 0.5260971 ,
        0.46497855, 0.        , 1.        , 0.45564517, 0.5121951 ,
        0.12007663, 0.45625   , 0.7692308 , 0.        ],
...
function demo(){
  ...
  let graph_plot4 = document.getElementById('div_predict_graph');
